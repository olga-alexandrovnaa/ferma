...
function ProductsList(props) {
  const onAddToCartHandler = useCallback((item, count) => {
    cartAddItem(item, count);
  }, [])

  const getCards = useCallback((products, id) => {
    let content = [];
    for (let i = id*3; (i < products.length) && (i<id+3); i++) {
      const item = products[i];
      content.push(<Card item={item} onAddToCart={onAddToCartHandler} key={i} />);
    }
    return content;
  }, []);
  
  const getProducts = useCallback((products, header) => {
    let content = [];
    let Bottons = [];
    let Cards = [];
